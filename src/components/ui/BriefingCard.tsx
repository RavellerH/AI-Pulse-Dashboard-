    ...
    <div className="bg-surface-1 border border-border-default rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-border-default flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <span className={`text-xs font-medium px-2 py-1 rounded-lg border shrink-0 ${windowClass}`}>
            {WINDOW_LABEL[briefing.window] ?? briefing.window}
          </span>
          <h3 className="text-sm font-semibold text-text-primary truncate">{briefing.title}</h3>
        </div>
        <p className="text-xs font-mono text-text-muted shrink-0">{formatRelative(briefing.generatedAt)}</p>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {briefing.highlights.length > 0 && (
          <div>
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wider mb-2.5">Highlights</p>
        ...
        {briefing.topEntities.length > 0 && (
          <div>
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wider mb-2.5">Most Discussed</p>
            <div className="flex flex-wrap gap-1.5">
              {briefing.topEntities.map((e) => (
                <span key={e} className="text-xs bg-surface-2 border border-border-default text-text-secondary px-2.5 py-1 rounded-lg">
                  {e}
                </span>
              ))}
            </div>
        ...
        {briefing.debates.length > 0 && (
          <div>
            <p className="text-[11px] font-semibold text-text-muted uppercase tracking-wider mb-2.5">Debates</p>
            <ul className="space-y-1.5">
              {briefing.debates.map((d, i) => (
                <li key={i} className="text-xs text-text-muted flex items-start gap-2">
                  <span className="text-delayed shrink-0 mt-0.5">⚡</span>
